...
class SurveyDataSet(object):
	"""
	Represents a single survey's data set.  Initialized with a single argument,
	the filename of the CSV data set.
	"""
	...
	def __init__(self, dataset_filename):
		...
		with open(dataset_filename, 'rbU') as csv_file:
			...
			for variable in data:
				new_question = Question(name=variable, data=data[variable], parent_dataset=self)
				self.questions.append(new_question)

	def get_question(self, question_label):
		"""
		Returns the question matching the label.  Raises AttributeError if no
		matching question exists
		"""
		for q in self.questions:
			if q.name == question_label:
				return q
...
class Question(object):
	"""
	Represents a single question.  Initialized with name, a list of responses,
	and a reference to its parent study.
	"""
